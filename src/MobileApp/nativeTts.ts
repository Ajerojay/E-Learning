...
export function speakKidPrompt(
  text: string,
  options: { interrupt?: boolean; rate?: number; pitch?: number; onEnd?: () => void } = {}
) {
  if (!text.trim()) return;
  ...
  if (speakNative(text, { interrupt: options.interrupt !== false, rate: options.rate ?? 0.95, pitch: options.pitch ?? 1.3 })) {
